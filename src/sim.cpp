...
namespace Sim {
	...
	void load_state(std::string file_name, PositionList &positions, LinConstraintList &linconstraints, AngConstraintList &angconstraints) {

		std::ifstream state_file(file_name, std::ifstream::in);
		state_file >> originallength;
		size_t num_positions;
		state_file >> num_positions;
		positions.clear();
		...
		for (size_t i = 0; i < num_positions; i++) {
			...
			state_file >> vy;
			state_file >> ispinned;
			positions.push_back(Position(vec2(posx,posy), vec2(vx, vy), ispinned));
		}

		linconstraints.clear();
		size_t num_lconstraints;
		state_file >> num_lconstraints;
		for (size_t i = 0; i < num_lconstraints; i++) {
			size_t posa, posb;
			float analpha;
			state_file >> posa;
			state_file >> posb;
			state_file >> analpha;
			linconstraints.push_back(LinConstraint(positions.begin() + posa, positions.begin() + posb, analpha));
		}

		angconstraints.clear();

		size_t num_aconstraints;
		state_file >> num_aconstraints;

			for (size_t i = 0; i < num_aconstraints; i++) {
			...
			state_file >> posc;
			state_file >> analpha;
			angconstraints.push_back(AngConstraint(positions.begin() + posa, positions.begin() + posb, positions.begin() + posc, analpha));
		}
	...
	void save_state(std::string file_name, PositionList &positions, LinConstraintList &linconstraints, AngConstraintList &angconstraints) {

		std::ofstream state_file(file_name, std::fstream::out | std::fstream::trunc);

		state_file <<originallength << "\n";

		state_file << positions.size() << "\n";

		for (auto &pos: positions) 
			state_file << pos.position.x << " " << pos.position.y << " " << pos.velocity.x << " " << pos.velocity.y << " " <<  pos.pinned << "\n";

		state_file << linconstraints.size() << "\n";

		for (auto &lc: linconstraints) {

			size_t indexa = lc.pos1 - positions.begin();
			size_t indexb = lc.pos2 - positions.begin();
			float analpha = lc.alpha;

			state_file << indexa << " " << indexb << " " << analpha << "\n";
		}

		state_file << angconstraints.size() << "\n";
		for (auto &ac: angconstraints) {

			size_t indexa = ac.pos1 - positions.begin();
			size_t indexb = ac.pos2 - positions.begin();
			size_t indexc = ac.posc - positions.begin();
			float analpha = ac.alpha;

			state_file << indexa << " " << indexb << " "  << indexc << " " << analpha << "\n";
